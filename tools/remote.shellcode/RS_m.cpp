...
int main(int argc, char* argv[])
{
	...
	memAddress = (LPVOID)VirtualAllocEx(hProcess, NULL, shellcode_size, MEM_RESERVE | MEM_COMMIT, PAGE_READONLY);
	if (memAddress == NULL){
	...
	DWORD oldProtection;
	bstatus = VirtualProtectEx(hProcess, memAddress, shellcode_size, PAGE_EXECUTE_READWRITE, &oldProtection);
	if (!bstatus){
